...
<div id='editor'>
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
              ...
              <modal v-if="dialogShown==1" @close="dialogShown = -1">
                  ...
                  <div slot='body'>
                      <div class='row'>
                          <span class='col-md-2'>Length:</span> <select class='col-md-1' v-model="lineParams.selectedLength" ><option v-for='i in numbers'>{{i}}</option></select></div>
                        <div class='row'>
                          <span class='col-md-2'>Angle:</span> <select class='col-md-1' v-model='lineParams.selectedAngle'><option>15</option><option>30</option><option>60</option><option>90</option></select>
                        </div>
                  </div>
                  <div slot='footer'>
              ...
              <modal  v-if='dialogShown==2' @close="dialogShown = -1">
                  ...
                  <div slot='body' class='row'>
                    <span class='col-md-2'>Radius:</span> <span class='col-md-2'><select class='form-control' v-model="circleParams.selectedRadius" ><option v-for='i in numbers'>{{i}}</option></select></span></div>
              ...
              <modal  v-if='dialogShown==3' @close="dialogShown = -1">
                  ...
                  <div slot='body'>
                  <div class='row' >
                    <span class='col-md-2'>Width:</span> <span class='col-md-2'><select class='form-control' v-model="rectParams.selectedWidth" v-for='i in numbers'><option>{{i}}</option></select></span></div>
                  
                 
                      <div>
                          <span class='col-md-2'>Height:</span><span class='col-md-2'> <select class='form-control' v-model="rectParams.selectedHeight" v-for='i in numbers'><option>{{i}}</option></select></span></div>
                        </div> 
                        <div slot='footer'>
            ...
            <modal  v-if='dialogShown==4' @close="dialogShown = -1">
              ...
              <div slot='body'>
              <div >
                <span class='col-md-2'>Type:</span> <select class='col-md-2' v-model="chartParams.type" ><option>line</option><option>scatter</option></select></div>
              <div>Title: <input type='text' class='offset-md-2' v-model='chartParams.title'></div>
              <div>X Title: <input type='text' class='offset-md-2' v-model='chartParams.x_title'></div>
                  <div>
                      <span>Num points:</span> <input type='text' class='offset-md-2' v-model='chartParams.numpoints'></div>
                      <div>
                        <button class='btn btn-primary' v-on:click='showpoints' >Apply</button>
                    </div> 
                    <div v-for='point in chartParams.points'>
                       <div>
                         X: <input type="text" class='offset-md-2' v-model='point.x'>
                       </div>
                       <div>
                          Y: <input type="text" class='offset-md-2' v-model='point.y'>
                       </div>
                    </div>
           ...
           methods: {
               ...
               createImage: function(data)
               {
                ...
                $.get('./apis/ImageCreator.php', data,  function(res)
                {
                    ...
                    self.components.push({'type':'pict', 'src':   res.replace('..', '/blogeditor'), 'style':'border:5px solid blue'});
                    self.components.push({type: 'textelem', contenteditable:true, style: 'border: 3px solid black;'});
                    self.dialogShown=-1;
